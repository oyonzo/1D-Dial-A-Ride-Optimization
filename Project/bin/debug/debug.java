...
public class debug {
	...
	public final static double w1 = 0;
	public final static double w2 = 1;
	public final static double w3 = 1;
	...
	/*
	 * Takes in the list of randomized linear requests
	 */
	public static void changeWeight(List<LinearRequest> lrl) {
		...
		for(double i = 0; i<=10; i++) {
			for(double j = 0; j<=10; j++) {
				for(double k = 0; k<=10; k++) {
					...
					newAlg(temp,0,0,i,j,k);
					
					
					if(set.size() > current) {
//						System.out.println("Changing weights from: " + w1 + "," + w2 + "," + w3 + " to: " + i + "," + j + "," + k + ",") ;
						weights[0] = i;
						weights[1] = j;
						weights[2] = k;
						current = set.size();
	...
	public static void newAlg(List<LinearRequest> lrl, double time, int origin, double weight1, double weight2, double weight3) {
		...
		//remove all illegal requests
				...
				if (lrl.size()>0) {
					...
					l.setf(weight1, weight2, weight3, Math.abs((double)l.startPos - origin),l.finishTime,l.getX3(lrl));
					...
					newAlg(lrl,time,origin,weight1,weight2,weight3);//Recursive call
				}
	...
	public static void nervous2(List<LinearRequest> lrl) {
		//List<LinearRequest> result = new ArrayList<LinearRequest>();
		//Add End Later
		//Should be 0,1,1
		changeWeight(lrl);
		newAlg(lrl,0,0,weights[0],weights[1],weights[2]);
		makePath2(0,0,Integer.signum(set.get(0).startPos),new LinearRequest(0,0,0),0);
		goodShare();
